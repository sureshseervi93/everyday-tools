...
def compile_individual_embeds():
    base_dir = "."
    output_dir = os.path.join(base_dir, "google_sites_embeds")
    ...
    # Clean utils exports for inlining
    clean_utils = re.sub(r'export\s+', '', utils_content)

    print("Generating individual Google Sites widgets...")
    for tool in TOOLS:
        tool_id = tool['id']
        
        # Read the specific tool logic
        tool_js_path = os.path.join(base_dir, "js", "tools", f"{tool_id}.js")
        with open(tool_js_path, "r", encoding="utf-8") as f:
            tool_js = f.read()

        # Remove imports and clean exports in tool code
...
<head>
    ...
    <style>
        ...
        /* Hide sidebar/navbar layout wrapper restrictions since there is no site wrapper */
        .workspace-container {{
            max-width: 100%;
            margin: 0;
        }}
...
<body data-theme="light">
    ...
    <div class="workspace-container">
        ...
        <div class="editor-layout" style="display:flex; flex-direction:row; gap:20px; flex-wrap:wrap;">
            ...
            <!-- Workspace Main Area (Dropzone & Preview) -->
            <div style="flex:1; min-width:320px;">
                <!-- Dropzone -->
                <div class="dropzone" id="tool-dropzone" style="height:250px;">
                ...
                <div class="editor-workspace hidden" id="editor-workspace">
                    <div class="editor-main" id="editor-main-area" style="background:var(--bg-sidebar); border:1px solid var(--border-color); border-radius:12px; min-height:250px; padding:16px;">
                        <!-- Injected by JS -->
                    </div>
                </div>
            ...
            <!-- Options Panel Sidebar (Pre-spaced) -->
            <div id="options-sidebar-container" style="width:260px; flex-shrink:0;">
                <div class="editor-sidebar" style="height:auto; border:1px solid var(--border-color); border-radius:12px; padding:16px; background:var(--bg-sidebar);">
                    ...
                    <div id="dynamic-options-panel">
                        <!-- Options injected by tool modules -->
                    </div>
    ...
    <script>
        ...
        // 3. Mini page runtime controller
        (function() {{
            ...
            window.addEventListener('DOMContentLoaded', async () => {{
                ...
                // Render Workspace directly for no-file tools
...
"""
        ...
        output_filename = os.path.join(output_dir, f"{tool_id}_embed.html")
        with open(output_filename, "w", encoding="utf-8") as f:
            f.write(widget_html)
        print(f" - Created {output_filename}")
        
    print(f"Done! Standalone widgets created in {output_dir}")

if __name__ == "__main__":
    compile_individual_embeds()
